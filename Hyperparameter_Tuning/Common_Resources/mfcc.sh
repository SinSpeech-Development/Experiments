...
if [ $stage -le 1 ]; then
    ...
    echo "===== BEGIN : Validation Set MFCC feature extraction ====="    
    echo
    steps/make_mfcc.sh --nj $nj --cmd "$train_cmd" data/valid exp/make_mfcc/test $mfccdir
    steps/compute_cmvn_stats.sh data/valid exp/make_mfcc/valid $mfccdir
    echo
    echo "===== END: Validation Set MFCC feature extraction ====="
